...
class pd_processor:
    ...
    def do(self, input_pcm):
        ...
        Show_Spectrogram(self.spec, self.global_max)
    ...
    def detect_pitch(self):
        ...
        for freq in range(88):
            peaks, _ = signal.find_peaks(self.spec[:,freq], prominence = 400/(freq+2), height=100) ## 4~5사이 값이 적당한듯함
            #ends, _ = signal.find_peaks(self.spec[:,freq], prominence = 0.1, hz_th=0.1)
            #ends : 저걸로가뭐야 쉼표 아예무음구간 나올대만 끊어줘
    ...
    def get_spectrogram(self, pcm):
        ...
        for i in range(len(Zxx)):
            frame_peak_freqs, _= signal.find_peaks(Zxx[i], prominence = 10)
            for freq in frame_peak_freqs:
                if Zxx[i][freq] > self.spec_th and Zxx[i][freq]>spec[i][self.dict[freq]]:
                    spec[i][self.dict[freq]] = Zxx[i][freq]
