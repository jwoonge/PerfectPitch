...
class score:
    ...
    def detect_key(self):
        ...
        for accord_it in self.accords_left:
            if accord_it.vice==0:
                for note in accord_it.notes:
                    pitch_count[note.pitch] += 1
        for accord_it in self.accords_right:
            if accord_it.vice==0:
                for note in accord_it.notes:
                    pitch_count[note.pitch] += 1
    ...
    def make_wav(self, filename='output', test=False):
        from MeasureAccuracyModule.ScoreToWAV import score_to_wav
